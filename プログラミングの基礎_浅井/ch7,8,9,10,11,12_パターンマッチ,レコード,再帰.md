...
person_tは、blood=bのみでもperson_tの型推定がされた。
...
let rec seiretsu lst =
  let sorted = seiretsu_sort lst in
  match sorted with
      [] -> []
    | first :: rest ->
        first:: (
          match rest with
              []              -> []
            | next::rest_next -> if next.kana = first.kana then seiretsu rest_next
                                                else next :: seiretsu rest_next
        )
    ;;
